...
SENSORS = {
    "Midframe (Hasselblad X2D)": {"w": 43.8, "h": 32.9, "cf": 0.79, "fl_range": (20.0, 300.0)},
    "Full Frame": {"w": 36.0, "h": 24.0, "cf": 1.00, "fl_range": (10.0, 800.0)},
    "APS-C": {"w": 23.6, "h": 15.6, "cf": 1.52, "fl_range": (8.0, 600.0)},
    "M43": {"w": 17.3, "h": 13.0, "cf": 2.00, "fl_range": (7.0, 400.0)},
    "1/0.98 inch (Phone LYT-900)": {"w": 13.2, "h": 8.8, "cf": 2.73, "fl_range": (4.0, 100.0)},
    "1/1.28 inch (Phone IMX903)": {"w": 9.8, "h": 7.4, "cf": 3.67, "fl_range": (3.0, 35.0)},
    "1/1.4 inch (Phone HP9)": {"w": 8.4, "h": 6.3, "cf": 4.28, "fl_range": (3.0, 35.0)}
}

APERTURES = [0.95, 1.2, 1.4, 1.7, 1.8, 2.0, 2.4, 2.8, 3.2, 4.0, 4.5, 5.0, 5.6, 6.3, 8.0, 11.0, 16.0, 22.0]
...
class PhotoCalculator(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Photography Sensor & Exposure Calculator")
        self.geometry("550x750")
        self.configure(padx=20, pady=20)

        # Variables
        self.sensor_var = tk.StringVar(value="Full Frame")
    ...
    def create_widgets(self):
        ...
        # --- Sensor Selection ---
        tk.Label(self, text="Sensor Size:", font=("Arial", 10, "bold")).grid(row=row, column=0, sticky="w", pady=5)
        ...
        # --- Continuous Sliders ---
        # Focal Length
        tk.Label(self, text="Actual Focal Length (mm):", font=("Arial", 10, "bold")).grid(row=row, column=0, sticky="w", pady=(10, 0))
        ...
        tk.Label(btn_frame, text="Quick Equiv. Primes:", font=("Arial", 8, "italic")).pack(side="left", padx=(0, 5))
        ...
        # Subject Distance
        tk.Label(self, text="Subject Distance (meters):", font=("Arial", 10, "bold")).grid(row=row, column=0, sticky="w", pady=10)
        ...
        # --- Discrete Sliders ---
        # Aperture
        tk.Label(self, text="Aperture (f-stop):", font=("Arial", 10, "bold")).grid(row=row, column=0, sticky="w", pady=10)
        ...
        # Shutter Speed
        tk.Label(self, text="Shutter Speed:", font=("Arial", 10, "bold")).grid(row=row, column=0, sticky="w", pady=10)
    ...
    def update_calculations(self, *args):
        ...
        output =  f"--- SENSOR EQUIVALENTS ---\n"
        output += f"FF Equiv. Focal Length : {eq_fl:.1f} mm\n"
        output += f"FF Equiv. Aperture     : f/{eq_aperture:.1f} (Total Light/DoF)\n\n"
        
        output += f"--- LIGHT / EXPOSURE ---\n"
        output += f"Exposure Value (EV100) : {ev_100:.2f} EV\n"
        output += f"Req. Scene Brightness  : {ev_scene:.2f} EV\n"
        output += f"(Lower EV = darker scene/more light gathered)\n\n"
        
        output += f"--- DEPTH OF FIELD ---\n"
        output += f"Hyperfocal Distance    : {hyperfocal_mm/1000:.2f} m\n"
        output += f"Near Limit             : {near_m:.2f} m\n"
        
        if far_m == float('inf'):
            output += f"Far Limit              : Infinity\n"
            output += f"Total Depth of Field   : Infinite\n"
        else:
            output += f"Far Limit              : {far_m:.2f} m\n"
            output += f"Total Depth of Field   : {total_dof:.2f} m\n"
